// Small (and fast) std::function implementation by Malte Skarupke.
// On x86-64 Clang 4, size goes from 48 B (std::function) to 32 B, while
// speedups are negligable in -O3 but about 20% faster in -O0.
/*
...
namespace rx
{
#ifndef RX_TARGET_BUILD_NO_CXX_EXCEPTIONS
...
# endif
  ...
  template<typename Result, typename... Arguments>
  struct is_valid_function_argument<func<Result (Arguments...)>, Result (Arguments...)>
  {
    static const bool value = false;
  };
...
#       ifndef RX_TARGET_BUILD_NO_CXX_RTTI
...
public:
  ...
  func(std::nullptr_t) FUNC_NOEXCEPT
  {
    initialize_empty();
  }
  func(func && other) FUNC_NOEXCEPT
  {
    initialize_empty();
    swap(other);
  }
  ...
  template<typename Allocator>
  func(std::allocator_arg_t, const Allocator &)
  {
    // ignore the allocator because I don't allocate
    initialize_empty();
  }
  template<typename Allocator>
  func(std::allocator_arg_t, const Allocator &, std::nullptr_t)
  {
    // ignore the allocator because I don't allocate
    initialize_empty();
  }
  template<typename Allocator, typename T>
  func(std::allocator_arg_t, const Allocator & allocator, T functor,
  ...
  template<typename Allocator>
  func(std::allocator_arg_t, const Allocator & allocator, const func & other)
    : call(other.call)
  {
    typedef typename std::allocator_traits<Allocator>::template rebind_alloc<func> MyAllocator;
    ...
    {
      detail::manager_type manager_for_function = &detail::get_default_manager<func, MyAllocator>();
      if (other.manager_storage.manager == manager_for_function)
      {
        detail::create_manager<func, MyAllocator>(manager_storage, MyAllocator(allocator));
  ...
  template<typename Allocator>
  func(std::allocator_arg_t, const Allocator &, func && other) FUNC_NOEXCEPT
  {
    // ignore the allocator because I don't allocate
    initialize_empty();
    swap(other);
  ...
  func & operator=(func other) FUNC_NOEXCEPT
  {
    swap(other);
    return *this;
  }
  ...
  {
    func(std::allocator_arg, allocator, functor).swap(*this);
  }
  
  void swap(func & other) FUNC_NOEXCEPT
  {
...
#   ifdef RX_TARGET_BUILD_NO_CXX_EXCEPTIONS
      return call != nullptr;
#   else
...
template<typename T>
bool operator==(std::nullptr_t, const func<T> & rhs) FUNC_NOEXCEPT
{
  return !rhs;
}
template<typename T>
bool operator==(const func<T> & lhs, std::nullptr_t) FUNC_NOEXCEPT
{
  return !lhs;
}
template<typename T>
...
template<typename T>
void swap(func<T> & lhs, func<T> & rhs)
{
  lhs.swap(rhs);
}
